...
export function AppSidebar({ view, onViewChange, unreadCount, onOpenNotifications }: AppSidebarProps) {
  ...
  const paletteDialog = isPaletteOpen ? createPortal(
    <div
      className="palette-popover palette-popover-portal"
      role="dialog"
      aria-label="Choose color palette"
      style={{ left: palettePosition.left, bottom: palettePosition.bottom }}
      ...
      <div className="palette-grid">
        {paletteOptions.map((option) => (
          <button
            ...
            title={option.label}
          >
            <span className="palette-swatch" /><span>{option.label}</span>{palette === option.value ? <Check size={13} /> : null}
          </button>
        ))}
      </div>
    </div>,
  ...
  return (
    <>
      <aside
        ...
        onMouseLeave={() => { if (!isPaletteOpen) collapseSidebar() }}
        onFocusCapture={() => setIsHovered(true)}
        onBlurCapture={(event) => {
          if (!isPaletteOpen && !event.currentTarget.contains(event.relatedTarget as Node | null)) collapseSidebar()
        }}
      >
        <SideNav
          ...
          header={
            <div className="sidebar-brand">
              <span className="sidebar-logo" aria-hidden="true"><PocketKnife size={18} strokeWidth={2.1} /></span>
              <div className="sidebar-brand-copy"><strong>FlowLane</strong><span>{membership?.workspace.name}</span></div>
            </div>
          }
          footer={
              ...
              <div className="sidebar-utility-row">
                {!isViewer ? (
                  <div className="sidebar-notification-action">
                    <IconButton label="Notifications" icon={<MessageSquare size={17} />} variant="ghost" size="sm" onClick={onOpenNotifications} />
                ...
                <div className="sidebar-palette-control" ref={paletteAnchorRef}>
                  <IconButton label="Choose color palette" icon={<PaletteIcon size={17} />} variant="ghost" size="sm" className={isPaletteOpen ? 'is-active' : undefined} onClick={() => { setIsHovered(true); setIsPaletteOpen((current) => !current) }} />
                </div>
              </div>
              <div className="sidebar-user">
                <span className="sidebar-avatar">{initial}</span>
                <div className="sidebar-user-copy"><strong>{displayName}</strong><span>{membership?.role}</span></div>
