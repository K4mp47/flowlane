...
interface ProjectsPanelProps { workspaceId: string; projects: Project[]; activeProjectId?: string | null; role: WorkspaceRole | null; onSelectProject: (projectId: string) => void; onChanged: () => Promise<void> | void }
...
export function ProjectsPanel({ workspaceId, projects, activeProjectId, role, onSelectProject, onChanged }: ProjectsPanelProps) {
  ...
  async function createProject(event: FormEvent) {
    event.preventDefault(); const name = newName.trim(); if (!name || !isAdmin) return
  ...
  async function saveRename(projectId: string) { const name = editingName.trim(); if (!name || !isAdmin) return; setError(null); setBusyId(projectId); const { error: updateError } = await supabase.from('projects').update({ name }).eq('id', projectId).eq('workspace_id', workspaceId); setBusyId(null); if (updateError) return setError(updateError.message); setEditingId(null); await onChanged() }
  async function setDefault(projectId: string) { if (!isAdmin) return; setError(null); setBusyId(projectId); const { error: defaultError } = await supabase.rpc('set_default_project', { _workspace_id: workspaceId, _project_id: projectId }); setBusyId(null); if (defaultError) return setError(defaultError.message); await onChanged() }
  async function deleteProject(projectId: string, name: string) {
    if (!isAdmin || !window.confirm(`Delete “${name}” and all of its tasks, workflow statuses, board view and project data? This cannot be undone.`)) return
    ...
    setBusyId(null); if (deleteError) return setError(deleteError.message)
    await onChanged()
  }

  return <section className="projects-page">
    <div className="projects-page-heading"><div><p className="eyebrow">Workspace projects</p><h2>Projects</h2><p className="muted">Projects own tasks and workflows. Each project gets its own default board view.</p></div><div className="project-count"><FolderKanban size={17} /><strong>{projects.length}</strong><span>{projects.length === 1 ? 'project' : 'projects'}</span></div></div>
