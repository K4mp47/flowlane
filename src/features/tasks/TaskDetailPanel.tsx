...
export function TaskDetailPanel({ task, role, currentUserId, taskType, assignees, profiles, members, onClose, onEdit, onChanged }: TaskDetailPanelProps) {
  ...
  async function deleteTask() {
    if (!can(role, 'task:delete') || !window.confirm(`Delete FL-${task.task_number} “${task.title}”? This cannot be undone.`)) return
  ...
  return (
    <motion.div className="task-peek-backdrop" onMouseDown={onClose} initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} transition={{ duration: .18 }}>
      <motion.aside className="task-peek task-detail-panel" onMouseDown={(event) => event.stopPropagation()} initial={{ x: '100%', opacity: .85 }} animate={{ x: 0, opacity: 1 }} exit={{ x: '100%', opacity: .8 }} transition={{ type: 'spring', stiffness: 420, damping: 38, mass: .75 }}>
        ...
        <div className="task-detail-heading-row"><h2>{task.title}</h2><div className="task-detail-heading-actions">{can(role, 'task:edit') ? <Button label="Edit" variant="secondary" size="sm" icon={<Edit3 size={14} />} onClick={onEdit} /> : null}{can(role, 'task:delete') ? <Button label="Delete" variant="danger" size="sm" icon={<Trash2 size={14} />} onClick={() => void deleteTask()} isLoading={isDeleting} /> : null}</div></div>
