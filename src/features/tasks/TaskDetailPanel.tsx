    ...
    <div className="task-peek-backdrop" onMouseDown={onClose}>
      <aside className="task-peek task-detail-panel" onMouseDown={(event) => event.stopPropagation()}>
        ...
        <div className="peek-section">
          ...
          <div className="checklist-list ui-checklist-list">{checklist.map((item) => <div className={item.is_completed ? 'ui-checklist-item completed' : 'ui-checklist-item'} key={item.id}><CheckboxInput label={item.content} value={item.is_completed} isReadOnly={isReadOnly} size="sm" width="100%" onChange={() => void toggleChecklist(item)} />{can(role, 'checklist:edit') ? <IconButton label="Delete checklist item" icon={<Trash2 size={13} />} variant="ghost" size="sm" onClick={() => void deleteChecklistItem(item.id)} /> : null}</div>)}</div>
          {can(role, 'checklist:edit') ? <div className="inline-editor-row ui-inline-editor"><TextInput label="Checklist item" isLabelHidden value={checklistText} onChange={setChecklistText} placeholder="Add checklist item…" /><Button label="Add" size="sm" variant="secondary" icon={<Plus size={14} />} onClick={() => void addChecklistItem()} isDisabled={!checklistText.trim()} /></div> : null}
        </div>
