...
export function TeamPanel({ workspaceId, project, boards, activeBoard, profiles, workspaceRole, accessRole, onInvited }: TeamPanelProps) {
  ...
  return <section className="team-page">
    ...
    {canManage ? <form className="invite-card team-directory-card" onSubmit={inviteMember}>
      ...
      <div className="invite-role-selector"><Selector label="Scope" isLabelHidden options={scopeOptions} value={scope} onChange={(value) => setScope(value as 'PROJECT' | 'BOARD')} width="100%" /></div>
      {scope === 'BOARD' ? <div className="invite-role-selector"><Selector label="Board" isLabelHidden options={boardOptions} value={boardId} onChange={setBoardId} width="100%" /></div> : null}
