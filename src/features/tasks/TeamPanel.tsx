...
export function TeamPanel({ workspaceId, profiles, members, onInvited }: TeamPanelProps) {
  ...
  const rows = useMemo(() => members.map((member) => ({ ...member, profile: profiles.find((profile) => profile.id === member.user_id) })), [members, profiles])
  ...
  async function inviteMember(event: FormEvent) {
    ...
    const { data, error: invokeError } = await supabase.functions.invoke('invite-member', { body: { workspaceId, email: normalizedEmail, role, redirectTo: window.location.origin } })
    if (invokeError || data?.error) { setError(invokeError?.message ?? String(data?.error)); setIsLoading(false); return }
    setEmail(''); setResults([]); setMessage(`Invitation sent to ${normalizedEmail} as ${role}.`); await onInvited(); setIsLoading(false)
  }

  async function removeMember(userId: string, label: string) {
    if (!window.confirm(`Remove ${label} from this workspace? Their task assignments in this workspace will also be removed.`)) return
    setError(null); setMessage(null); setRemovingUserId(userId)
    const { error: removeError } = await supabase.rpc('remove_workspace_member', { _workspace_id: workspaceId, _user_id: userId })
    setRemovingUserId(null)
    if (removeError) return setError(removeError.message)
    setMessage(`${label} no longer has access to this workspace.`)
    await onInvited()
  }

  return <section className="team-page">
    <div className="team-page-heading"><div><p className="eyebrow">Workspace access</p><h2>Team</h2><p className="muted">People added here can collaborate on the projects inside this workspace.</p></div><div className="team-count"><Users size={17} /><strong>{members.length}</strong><span>people</span></div></div>
    <form className="invite-card team-directory-card" onSubmit={inviteMember}>
      <div className="invite-card-icon"><Search size={19} /></div><div className="invite-copy"><strong>Add someone to this workspace</strong><span>Registered users can be added immediately; new users can be invited by email.</span></div>
      <div className="invite-email team-directory-search"><TextInput label="Search registered email" isLabelHidden type="email" value={email} onChange={(value) => { setEmail(value); setError(null); setMessage(null) }} placeholder="Search email…" startIcon={Search} />{email.trim().length >= 2 ? <div className="team-directory-results">{isSearching ? <div className="team-directory-state">Searching FlowLane users…</div> : null}{!isSearching && results.map((result) => <div className="team-directory-result" key={result.id}><span className="avatar-circle">{(result.displayName || result.email).slice(0, 1).toUpperCase()}</span><div className="team-directory-result-copy"><strong>{result.displayName || result.email.split('@')[0]}</strong><span>{result.email}</span></div><Button label="Add" variant="secondary" icon={<UserPlus size={15} />} isLoading={addingUserId === result.id} onClick={() => void addRegisteredUser(result)} /></div>)}{!isSearching && results.length === 0 ? <div className="team-directory-state">No registered user found. You can send an email invitation instead.</div> : null}</div> : null}</div>
      <div className="invite-role-selector"><Selector label="Role" isLabelHidden options={roleOptions} value={role} onChange={(value) => setRole(value as WorkspaceRole)} width="100%" /></div><Button label="Invite by email" variant="primary" type="submit" icon={<MailPlus size={16} />} isLoading={isLoading} />
    </form>
