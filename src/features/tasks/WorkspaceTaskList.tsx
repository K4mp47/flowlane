...
import type { Task, WorkflowStatus } from '../../types/domain'
...
export function WorkspaceTaskList({ workspaceId, userId, mode, onOpenTask }: WorkspaceTaskListProps) {
  ...
  const visible = tasks.filter((task) => { if (mode === 'mine' && !assignedIds.has(task.id)) return false; if (mode === 'today') { if (!assignedIds.has(task.id)) return false; const status = statusById.get(task.status_id); if (status?.is_terminal || !task.due_date) return false; return dayKey(task.due_date) <= today } return true }).sort((a, b) => { const aDue = a.due_date ? new Date(a.due_date).getTime() : Number.MAX_SAFE_INTEGER; const bDue = b.due_date ? new Date(b.due_date).getTime() : Number.MAX_SAFE_INTEGER; return aDue - bDue || b.updated_at.localeCompare(a.updated_at) })
  ...
  return <section className="workspace-task-page">
    ...
    <div className="workspace-task-table" role="table"><div className="workspace-task-row workspace-task-table-head" role="row"><span>Task</span><span>Project</span><span>Status</span><span>Due</span><span>Priority</span></div>{visible.map((task) => { const project = projectById.get(task.project_id); const status = statusById.get(task.status_id); const overdue = Boolean(task.due_date && dayKey(task.due_date) < today && !status?.is_terminal); return <button className="workspace-task-row" type="button" key={task.id} onClick={() => onOpenTask(task)} role="row"><span className="workspace-task-primary"><span className="task-reference">FL-{task.task_number}</span><strong>{task.title}</strong>{task.is_blocked ? <small><CircleAlert size={13} />Blocked</small> : null}</span><span><FolderKanban size={14} />{project?.name ?? 'Project'}</span><span><span className={`task-status-label workflow-tone-${statusTone(status)}`}>{status?.name ?? 'Status'}</span></span><span className={overdue ? 'workspace-task-due overdue' : 'workspace-task-due'}><CalendarDays size={14} />{task.due_date ? new Date(task.due_date).toLocaleDateString() : 'No deadline'}</span><span>{task.priority ? <Badge label={task.priority} variant={task.priority === 'URGENT' ? 'red' : task.priority === 'HIGH' ? 'orange' : 'neutral'} /> : '—'}</span></button> })}{!visible.length ? <div className="workspace-task-empty"><UserRoundCheck size={24} /><strong>Nothing needs attention here</strong><span>{mode === 'today' ? 'No assigned tasks are overdue or due today.' : 'No matching tasks yet.'}</span></div> : null}</div>
  </section>
}
