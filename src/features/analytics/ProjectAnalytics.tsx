...
interface ProjectAnalyticsProps { workspaceId: string; boards: Board[] }
...
export function ProjectAnalytics({ workspaceId, boards }: ProjectAnalyticsProps) {
  ...
  useEffect(() => {
    ...
    void load()
    const channel = supabase.channel(`analytics-${workspaceId}`).on('postgres_changes', { event: '*', schema: 'public', table: 'tasks', filter: `workspace_id=eq.${workspaceId}` }, () => void load()).on('postgres_changes', { event: '*', schema: 'public', table: 'task_assignees' }, () => void load()).subscribe()
  ...
  return (
    <section className="analytics-page">
      <header className="analytics-heading"><div><p className="eyebrow">Workspace intelligence</p><h2>Project analytics</h2><p>Live delivery health across every project in this workspace.</p></div><div className="analytics-completion-ring"><strong>{analytics.completionRate}%</strong><span>complete</span></div></header>
      <div className="analytics-kpi-grid"><Metric icon={<ListTodo size={18} />} label="Total tasks" value={analytics.total} /><Metric icon={<CheckCircle2 size={18} />} label="Completed" value={analytics.completed} /><Metric icon={<Activity size={18} />} label="Active work" value={analytics.inProgress} /><Metric icon={<AlertTriangle size={18} />} label="Overdue" value={analytics.overdue} emphasis={analytics.overdue > 0} /><Metric icon={<Clock3 size={18} />} label="Blocked" value={analytics.blocked} emphasis={analytics.blocked > 0} /><Metric icon={<Users size={18} />} label="Unassigned" value={analytics.unassigned} emphasis={analytics.unassigned > 0} /></div>
      <div className="analytics-layout">
        <article className="analytics-card"><div className="analytics-card-heading"><div><span className="eyebrow">Workflow</span><h3>Tasks by stage</h3></div><span>{analytics.total} tasks</span></div><div className="stage-bars">{analytics.byStage.map((entry) => { const width = analytics.total ? Math.max((entry.count / analytics.total) * 100, entry.count ? 4 : 0) : 0; return <div className="stage-bar-row" key={entry.stage}><div><span>{entry.label}</span><strong>{entry.count}</strong></div><div className="stage-bar-track"><span className={`stage-bar-fill stage-${entry.stage.toLowerCase().replace('_', '-')}`} style={{ width: `${width}%` }} /></div></div> })}</div></article>
        <article className="analytics-card"><div className="analytics-card-heading"><div><span className="eyebrow">Projects</span><h3>Delivery progress</h3></div><span>{boards.length} projects</span></div><div className="project-progress-list">{analytics.byBoard.map((board) => <div className="project-progress-row" key={board.id}><div className="project-progress-copy"><strong>{board.name}</strong><span>{board.done} of {board.total} completed</span></div><div className="project-progress-meter"><span style={{ width: `${board.rate}%` }} /></div><strong className="project-rate">{board.rate}%</strong></div>)}{!analytics.byBoard.length ? <div className="analytics-empty">Create a project to start collecting analytics.</div> : null}</div></article>
      </div>
    </section>
  )
}
