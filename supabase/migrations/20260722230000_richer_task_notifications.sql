...
declare
  ...
  v_message text;
begin
  select p.workspace_id, p.name into v_workspace_id, v_project_name from public.projects p where p.id = new.project_id;
  ...
  if new.status_id is distinct from old.status_id then
    ...
    if v_new_status.notify_on_enter then
      perform app_private.notify_workspace_members_for_task_event(
        v_workspace_id,new.id,v_actor,'STATUS_CHANGE','Task moved to ' || v_new_status.name,
...
begin
  ...
  if v_user is distinct from v_actor then
    ...
    values (
      v_workspace_id,v_user,new.task_id,'ASSIGNMENT','Task assigned to you',
